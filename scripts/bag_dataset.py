...
class BagDataset(object):
    ...
    def __init__(self, bag_info):
        ...
        self.topics = {'camera1': '/camera1/image_color/compressed'}
    ...
    def parse_time(self, t):
        ts, _ = self.ds.fetch_near(self.topics['camera1'], t, limit=1)[0]
        return ts


class MDBagDataset(BagDataset):

    def __init__(self, bag_info):
        super(MDBagDataset, self).__init__(bag_info)
    ...
    def postprocess(self, data):
        data = {k: Image.open(BytesIO(v.data)) for k,v in data.items()}
        return data
