...
from monodepth2.networks import ResnetEncoder, DepthDecoder, PoseDecoder, PoseCNN
from monodepth2.networks.layers import SSIM, BackprojectDepth, Project3D
from monodepth2.data import make_data_loader


class Trainer:
    def __init__(self, cfg):
        self.log_path = cfg.OUTPUT_DIR

        assert cfg.INPUT.HEIGHT % 32 == 0, "'height' must be a multiple of 32"
        assert cfg.INPUT.WIDTH % 32 == 0, "'width' must be a multiple of 32"

        self.device = cfg.MODEL.DEVICE
        self.scales = cfg.MODEL.SCALES
        self.frame_ids = cfg.MODEL.FRAME_IDS
        self.num_layers = cfg.MODEL.NUM_LAYERS
        self.weights_init = cfg.MODEL.WEIGHTS_INIT
        self.batch_size = cfg.SOLVER.IMS_PER_BATCH

        self.num_scales = len(self.scales)
        self.num_input_frames = len(self.frame_ids)
        self.num_pose_frames = 2 if cfg.MODEL.POSE_MODEL_INPUT == "pairs" else self.num_input_frames

        assert self.frame_ids[0] == 0, "frame_ids must start with 0"

        self.use_stereo = cfg.MODEL.USE_STEREO
    ...
    def build_model(self, cfg):
        ...
        if self.use_pose_net:
            if cfg.MODEL.POSE_MODEL_TYPE == "separate_resnet":
            ...
            elif cfg.MODEL.POSE_MODEL_TYPE == "shared":
                self.models["pose"] = PoseDecoder(
                    self.models["encoder"].num_ch_enc, self.num_pose_frames)

            elif cfg.MODEL.POSE_MODEL_TYPE == "posecnn":
                self.models["pose"] = PoseCNN(
                    self.num_input_frames if cfg.MODEL.POSE_MODEL_INPUT == "all" else 2)

            self.models["pose"].to(self.device)
            self.parameters_to_train += list(self.models["pose"].parameters())

        if cfg.MODEL.USE_PREDICTIVE_MASK:
            assert cfg.MODEL.DISABLE_AUTOMASKING, \
